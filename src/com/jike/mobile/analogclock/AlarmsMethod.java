...
//	        setStatusBarIcon(context, true);
	     ...
	     * @param atTimeInMillis
	     */
	    public static void updateAlarmTime(Context context,final Alarm alarm,
	            final long atTimeInMillis){
	    	ContentValues values = new ContentValues(1);
...
//	                    }
	   ...
	   public static void disableSnoozeAlert(Context context,final int AlarmId) {
	        AlarmManager am = (AlarmManager)
	                context.getSystemService(Context.ALARM_SERVICE);
	        PendingIntent sender = PendingIntent.getBroadcast(
	                context, AlarmId, new Intent(SNOOZE_ALARM_ALERT_ACTION),
...
//	    }
	    ...
	    static void disableAlert(Context context,int ID) {
	        AlarmManager am = (AlarmManager)
	                context.getSystemService(Context.ALARM_SERVICE);
	        PendingIntent sender = PendingIntent.getBroadcast(
	                context, ID, new Intent(ALARM_ALERT_ACTION),
	                PendingIntent.FLAG_CANCEL_CURRENT);
...
//	        setStatusBarIcon(context, false);
	    ...
	    public static boolean isHaveSnoozeAlarm(Context context) {
	    	Cursor cursor = (context.getContentResolver()).query(Alarm.Columns.CONTENT_URI,
	                Alarm.Columns.ALARM_QUERY_COLUMNS, Alarm.Columns.WHERE_SNOOZED,
	                null, null);
	    	boolean isHave;
	    	if(cursor.moveToFirst()){
				isHave=true;
...
//	        return android.text.format.DateFormat.is24HourFormat(context);
	    	return
	            ...
	            .getBoolean(SettingsPreferenceActivity.KEY_HOUR_MODE,
	            		true);
	    }
	    ...
	    static void popAlarmSetToast(Context context, int hour, int minute,
	        ...
	        popAlarmSetToast(context,
	                AlarmsMethod.calculateAlarm(hour, minute, daysOfWeek)
	                .getTimeInMillis());
	    }
