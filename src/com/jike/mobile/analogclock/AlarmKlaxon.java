...
//                	          С
                	mMediaPlayer.reset();
//                	mMediaPlayer.setDataSource(this, alert);AlarmsMethod.getSoundRsId(alarm.soundId)
                	if (alarm.soundType==1) {
                		mMediaPlayer = MediaPlayer.create(mContext,AlarmsMethod.getSoundRsId(alarm.soundId));
					}
...
//		}
    ...
    private void startAlarm(final MediaPlayer player,Float volume)
...
//                  С
        Log.v("maxVolume*volume ="+ maxVolume +"*" +volume+ "(int) (maxVolume*volume)="+(int) (maxVolume*volume));
		mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, (int) (maxVolume*volume), 0);
		Log.v(mAudioManager.getStreamVolume(AudioManager.STREAM_MUSIC)+" audioManager.getStreamVolume(AudioManager.STREAM_MUSIC)");
...
//    private static final MediaPlayer mMediaPlayer;
    ...
    public void stop() {
        ...
        if (mPlaying) {
            ...
            // Stop audio playing
            if (mMediaPlayer != null) {
            	((AudioManager)getSystemService(Context.AUDIO_SERVICE)).setStreamVolume(AudioManager.STREAM_MUSIC, oriVolume, 0);
                mMediaPlayer.stop();
